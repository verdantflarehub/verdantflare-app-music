...
训练接口同步执行，由 Music MCP Server 校验项目输入、调用该接口并持久化返回的模型包 Artifact。客户端断开不等同于可靠的任务取消，因此调用方必须等待工具返回明确结果后再推进审核状态。
...
| 服务镜像 | `verdantflare-app:music-rvc-api-v0.2.1` |
...
```bash
docker build \
  -t verdantflare-app:music-rvc-api-v0.2.1 \
...
docker run --rm \
  ...
  verdantflare-app:music-rvc-api-v0.2.1
```
