...
实现锁定 `audio-separator==0.46.0`，先使用 `mel_band_roformer_big_beta4.ckpt` 分离，再使用 `dereverb_mel_band_roformer_anvuew_sdr_19.1729.ckpt` 去混响。模型由上游模型目录下载到 `/models/audio-separator` 挂载卷，不写入镜像。

```bash
docker run --rm --gpus all -p 8000:8000 \
  -v uvr-models:/models/audio-separator \
  verdantflare-app:music-uvr5-api-v0.1.0
```
