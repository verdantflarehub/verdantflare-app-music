...
### 2.1 Stable Whisper (`stable-ts`)

- 适合点：官方接口支持 `align(audio, text)`，直接对齐已知正确文本；`original_split=True` 可保留换行分段；中文按非空格语言分词。
- 代价与风险：Whisper 对歌声的注意力对齐仍需真实音频验收。
- 结论：**采用**。

### 2.2 WhisperX

- 适合点：成熟的 ASR、VAD 和词级音素对齐。
- 代价与风险：标准流程是先转写再对齐；中文需额外语言特定 wav2vec2 模型，与“已知批准歌词”的边界不如 Stable Whisper 直接。
- 结论：不采用。

### 2.3 Montreal Forced Aligner

- 适合点：专业的语音强制对齐系统。
- 代价与风险：需中文词典、声学模型、Kaldi/Pynini/OpenFST 运行时；部署更重，且仍需单独评估歌声。
- 结论：不采用。
